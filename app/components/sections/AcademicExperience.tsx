...
const accents = [
  {
    ...
    dotGlow: "shadow-[0_0_0_6px_rgba(139,92,246,0.12)] dark:shadow-[0_0_0_6px_rgba(59,130,246,0.15)]",
    dotPulse: "bg-violet-400/20 dark:bg-blue-400/15",
    stripe: "from-violet-500/10 to-transparent dark:from-blue-500/10 dark:to-transparent",
    leftBorder: "border-violet-200 dark:border-blue-500/30",
    year: "bg-violet-50 text-violet-600 border-violet-200 dark:bg-blue-500/10 dark:text-blue-300 dark:border-blue-500/20",
    icon: "text-violet-500 dark:text-blue-400",
    iconBg: "bg-violet-50 dark:bg-blue-500/10",
    badge: "bg-violet-50 text-violet-600 border-violet-100 dark:bg-blue-500/10 dark:text-blue-300 dark:border-blue-500/20",
  ...
  },
  {
    ...
    dotGlow: "shadow-[0_0_0_6px_rgba(16,185,129,0.12)] dark:shadow-[0_0_0_6px_rgba(52,211,153,0.15)]",
    dotPulse: "bg-emerald-400/20 dark:bg-emerald-400/15",
    stripe: "from-emerald-500/10 to-transparent dark:from-emerald-500/10 dark:to-transparent",
    leftBorder: "border-emerald-200 dark:border-emerald-500/30",
    year: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/20",
  ...
  },
  {
    ...
    dotGlow: "shadow-[0_0_0_6px_rgba(245,158,11,0.12)] dark:shadow-[0_0_0_6px_rgba(251,191,36,0.15)]",
    dotPulse: "bg-amber-400/20 dark:bg-amber-400/15",
    stripe: "from-amber-500/10 to-transparent dark:from-amber-500/10 dark:to-transparent",
    leftBorder: "border-amber-200 dark:border-amber-500/30",
    year: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-300 dark:border-amber-500/20",
...
export default function AcademicExperience() {
  ...
  return (
    <section
      id="experiencia"
      className="relative bg-slate-100 px-6 py-28 transition-colors duration-300 md:px-10 lg:px-16 dark:bg-slate-950"
    >
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute -left-6 top-6 z-10 md:-left-10">
                    ...
                    >
                      <span className={`absolute h-7 w-7 animate-ping rounded-full opacity-20 ${a.dotPulse}`} />
                      <span className={`relative flex h-[13px] w-[13px] rounded-full ${a.dot} ${a.dotGlow}`} />
                    </motion.div>
                  </div>
                  ...
                  >
                    ...
                    <div className={`pointer-events-none absolute inset-0 bg-gradient-to-r ${a.stripe} opacity-0 transition-opacity duration-500 group-hover:opacity-100`} />

                    <div className="relative flex flex-col gap-5 p-6 md:flex-row md:gap-8 md:p-8">

                      {/* Columna izquierda: icono + meta */}
                      <div className="flex shrink-0 flex-row items-start gap-4 md:flex-col md:items-center md:gap-3 md:w-28">
                        {/* Icono */}
                        <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3 ${a.iconBg}`}>
                          <GraduationCap strokeWidth={1.5} className={`h-6 w-6 ${a.icon}`} />
                        </div>

                        {/* Periodo + ubicación en columna */}
                        <div className="flex flex-col gap-1.5 md:items-center md:text-center">
                          <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[10.5px] font-medium ${a.badge}`}>
                            <CalendarDays className="h-2.5 w-2.5" strokeWidth={2} />
                            {item.period}
                          </span>
                          <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[10.5px] font-medium ${a.badge}`}>
                      ...
                      {/* Divisor vertical */}
                      <div className={`hidden w-px self-stretch md:block ${a.line} opacity-30`} />

                      {/* Columna derecha: contenido */}
                      <div className="flex-1 min-w-0">
                        <h3 className={`mb-1 text-[1.05rem] font-bold leading-snug text-slate-900 transition-colors duration-300 dark:text-white ${a.title}`}>
                          ...
                          <ul className="grid gap-2 sm:grid-cols-2">
                            {(t.raw(`items.${item.key}.highlights`) as string[]).map(
                              (highlight: string, i: number) => (
                                ...
                                >
                                  <span className={`mt-[6px] h-1.5 w-1.5 shrink-0 rounded-full ${a.dot2}`} />
                                  {highlight}
                                </motion.li>
                              )
                            )}
